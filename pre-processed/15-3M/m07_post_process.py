...
class Post_process:
    ...
    def plot_phys(self):
        ...
        # Extract the LSTM prediction for different evapotranspiration values
        prediction_et_low = self.tester.lstm_output_test_et[0][0].cpu().detach()
        prediction_et_mid = self.tester.lstm_output_test_et[2][0].cpu().detach()
        prediction_et_high = self.tester.lstm_output_test_et[4][0].cpu().detach()
        ...
        # Plot the comparison of different evapotranspiration values
        fig, ax1 = plt.subplots(figsize=(16, 8))
        ax1.set_xlabel('Timestamp', size=18)
        ax1.set_ylabel('Ground water level [m]', size=18)
        ax1.plot(timestamp_plot, prediction_et_low, label='Low')
        ax1.tick_params(axis='y', labelcolor='k', labelsize=18)
        ax1.tick_params(axis='x', labelcolor='k', rotation=30, labelsize=18)
        ax1.plot(timestamp_plot, prediction_et_mid, label='Medium')
        ax1.plot(timestamp_plot, prediction_et_high, label='High')
        plt.title('SWL Prediction for Different Evapotranspiration', size=18)
        ax1.legend(loc='upper right', frameon=False, fontsize=18)
        fig.tight_layout()
        if self.cfg.save_model:
            fig.savefig(self.cfg.model_path + '\\' + self.cfg.model_name +
