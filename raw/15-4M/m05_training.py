...
class Training:
    ...
    def evaluate_train(self, inputs_swl_train, inputs_train, target_train):
        ...
        if self.cfg.physics_constraint:
            ...
            for i in range(5):
                inp_temp_t.append(inputs_train[rand_idx].clone().detach())
                
                # Modify the air temperature values
                inp_temp_t[i][:,:,2:4] = inputs_train[rand_idx,:,2:4].clone().detach() * (0.5 + i*0.25)
                
                # Calculate the LSTM outputs with the modified values and store
                # them in a list
                lstm_output_train_t.append(self.model(inputs_swl_train[rand_idx],
                ...
                if i >= 1:
                    lstm_train_loss += self.cfg.phys_mult *  torch.mean(torch.relu(
                        lstm_output_train_t[i] - lstm_output_train_t[i-1]))            
            ...
            # times the original values
            inp_temp_sd = []
            lstm_output_train_sd = []
            
            for i in range(5):
                inp_temp_sd.append(inputs_train[rand_idx].clone().detach())
                
                # Modify the sunshine duration values
                inp_temp_sd[i][:,:,4:6] = inputs_train[rand_idx,:,4:6].clone().detach() * (0.5 + i*0.25)
                
                # Calculate the LSTM outputs with the modified values and store
                # them in a list
                lstm_output_train_sd.append(self.model(inputs_swl_train[rand_idx],
                ...
                if i >= 1:
                    lstm_train_loss += self.cfg.phys_mult *  torch.mean(torch.relu(
                        lstm_output_train_sd[i] - lstm_output_train_sd[i-1]))            
            ...
            # times the original values
            inp_temp_rh = []
            lstm_output_train_rh = []
            
            for i in range(5):
                inp_temp_rh.append(inputs_train[rand_idx].clone().detach())
                
                # Modify the relative humidity values
                inp_temp_rh[i][:,:,6:8] = inputs_train[rand_idx,:,6:8].clone().detach() * (0.5 + i*0.25)
                
                # Calculate the LSTM outputs with the modified values and store
                # them in a list
                lstm_output_train_rh.append(self.model(inputs_swl_train[rand_idx],
                ...
                if i >= 1:
                    lstm_train_loss += self.cfg.phys_mult *  torch.mean(torch.relu(
                        lstm_output_train_rh[i-1] - lstm_output_train_rh[i]))            
            ...
            # times the original values
            inp_temp_wv = []
            lstm_output_train_wv = []
            
            for i in range(5):
                inp_temp_wv.append(inputs_train[rand_idx].clone().detach())
                
                # Modify the wind velocity values
                inp_temp_wv[i][:,:,8:10] = inputs_train[rand_idx,:,8:10].clone().detach() * (0.5 + i*0.25)
                
                # Calculate the LSTM outputs with the modified values and store
                # them in a list
                lstm_output_train_wv.append(self.model(inputs_swl_train[rand_idx],
                ...
                if i >= 1:
                    lstm_train_loss += self.cfg.phys_mult *  torch.mean(torch.relu(
                        lstm_output_train_wv[i] - lstm_output_train_wv[i-1]))
