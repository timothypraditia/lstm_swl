...
class Initialize_Data:
    ...
    def __init__(self, cfg):
        ...
        cfg.count_train = round(len(self.data) * cfg.percentage_train) - 1
        cfg.count_vali = round(len(self.data)* cfg.percentage_vali) - 1
        cfg.count_test = round(len(self.data)* cfg.percentage_test) - 1
        
        cfg.batch_size_train = round(cfg.count_train - max(cfg.in_p, cfg.in_et, cfg.in_swl)
                                     - cfg.forecast_horizon)
        cfg.batch_size_vali = round(cfg.count_vali - max(cfg.in_p, cfg.in_et, cfg.in_swl)
                                    - cfg.forecast_horizon)
        cfg.batch_size_test = round(cfg.count_test - max(cfg.in_p, cfg.in_et, cfg.in_swl)
                                    - cfg.forecast_horizon)
    ...
    def get_data_stats(self, cfg):
        ...
        cfg.min_p = torch.min(self.inp_p) - 0.15
        cfg.min_et = torch.min(self.inp_et) - 0.15
        cfg.min_w5 = torch.min(self.inp_w5) - 0.15
        cfg.min_w6 = torch.min(self.inp_w6) - 0.15
        
        # Find maximum value of each variable
        cfg.max_swl = torch.max(self.inp_swl) + 0.15
        cfg.max_p = torch.max(self.inp_p) + 0.15
        cfg.max_et = torch.max(self.inp_et) + 0.15
        cfg.max_w5 = torch.max(self.inp_w5) + 0.15
        cfg.max_w6 = torch.max(self.inp_w6) + 0.15
...
class Load_Data(Dataset):
    ...
    def __init__(self, dataset, cfg, training=False, validation=False, testing=False):
        ...
        if training:
            ...
            self.count_start = cfg.count_train - cfg.forecast_horizon
            self.count_end = cfg.count_train + cfg.count_vali
        elif testing:
            self.batch_size = cfg.batch_size_test
            self.count_start = cfg.count_train + cfg.count_vali - cfg.forecast_horizon
        ...
        # elif validation:
        #     self.batch_size = cfg.batch_size_vali
        #     self.count_start = cfg.count_train
        #     self.count_end = cfg.count_train + cfg.count_vali
        # elif testing:
        #     self.batch_size = cfg.batch_size_test
        #     self.count_start = cfg.count_train + cfg.count_vali
        #     self.count_end = cfg.count_train + cfg.count_vali + cfg.count_test
    ...
    def create_copy(self, dataset):
        ...
        self.inp_swl = copy.deepcopy(dataset.inp_swl)
    ...
    def reshape_data(self):
        ...
        #self.inputs_swl = torch.zeros(self.batch_size, 1, self.cfg.input_size_swl)
        self.inputs_swl = torch.zeros(self.batch_size, 1, 1)
        # self.inputs = torch.zeros(self.batch_size,self.cfg.forecast_horizon - 1, self.cfg.input_size)
        self.inputs = torch.zeros(self.batch_size,self.cfg.forecast_horizon, self.cfg.input_size)
        self.target = torch.zeros(self.batch_size, 1, self.cfg.forecast_horizon)
        
        for i in range(self.batch_size):
            # self.inputs_swl[i,0] = torch.cat((self.inp_swl[i : i + self.cfg.in_swl],
            #     self.inp_p[i : i + self.cfg.in_p + self.cfg.in_p_forecast],
            #     self.inp_et[i : i + self.cfg.in_et + self.cfg.in_et_forecast],
            #     self.inp_w5[i : i + self.cfg.in_w5 + self.cfg.in_w_forecast],
            #     self.inp_w6[i : i + self.cfg.in_w6 + self.cfg.in_w_forecast])).squeeze()
            
            self.inputs_swl[i,0] = self.inp_swl[i]
            
            # for t in range(1,self.cfg.forecast_horizon):
            #     self.inputs[i,t-1] = torch.cat(
            ...
            self.target[i,0] = self.tar_swl[i + 1 : i + 1 + self.cfg.forecast_horizon].squeeze()
